...
from lib.chat.setup import cohere
...
def read_input_files(file_paths):
    ...
    for file_path in file_paths:
        with open(file_path, "r", encoding="utf-8") as file:
            combined_articles.append(file.read())
...
def create_vectordb_from_docs(docs, persist_directory):
    vectordb = Chroma.from_documents(documents=docs, embedding=cohere, persist_directory=persist_directory)
    vectordb.persist()
    return vectordb
